...
const QRIcon = () => {
  return (<svg xmlns="http://www.w3.org/2000/svg" width="26.563" height="26.514" viewBox="0 0 26.563 26.514">
    ...
    <path d="M19.13 20.979v-2.133h2.133v-2.189H19.13v-2.132h-2.189v2.133h-2.132v2.189h2.133v2.133z" dataName="Path 10" />
    <path d="M25.86 21.252h-2.133v-2.133h-2.189v2.133h-2.132v2.189h2.133v2.133h2.189v-2.133h2.133z" dataName="Path 11" />
...
export default ({ openScanner, total, ens, address, changeView }) => {
  ...
  let topLeft = (<div className="hdr-UserInfoTop" onClick={() => changeView('receive')}>
    <div className="hdr-UserInfoTop-Avatar">{blockieDisplay}</div>
    <div className="hdr-UserInfoTop-Name">{isLoading ? "Loading..." : name}</div>
  </div>)

  let bottomRight = (
    <div className="sw-ButtonScan" onClick={() => { openScanner({ view: "send_to_address" }) }}>
      {QRIcon()}
    </div>
  )
